//! Запись аудио-дорожки канала в WAV (моно, 16-бит PCM) во время «кола».
//!
//! Каналы (`audio.rs`) декодируют f32 PCM @44100; здесь мы конвертируем в s16 и пишем в
//! WAV-файл. Размеры в заголовке WAV известны только в конце, поэтому пишем заголовок с
//! нулями, а при финализации (Drop) досоставляем размеры через seek. Так даже при
//! аварийном закрытии канала файл остаётся валидным (Drop гарантирован).

use std::fs::File;
use std::io::{Seek, SeekFrom, Write};
use std::path::Path;

/// 44-байтный заголовок WAV с нулевыми размерами (патчатся в finalize).
const HEADER_LEN: u64 = 44;

/// Пишущий WAV-файл одной дорожки. Финализирует заголовок при Drop.
pub struct WavRecorder {
    file: File,
    /// Сколько байт аудио-данных уже записано (для полей размера).
    data_bytes: u32,
}

impl WavRecorder {
    /// Создать WAV-файл (моно, 16-бит, `rate` Гц) и записать заголовок-заготовку.
    ...
    /// Заголовок WAV для моно/16-бит/`rate` с заданным размером данных.
    fn header(rate: u32, data_bytes: u32) -> [u8; 44] {
        let byte_rate = rate * 2; // channels(1) * bits(16)/8
        let mut h = [0u8; 44];
        ...
        h[22..24].copy_from_slice(&1u16.to_le_bytes()); // каналов: 1
    ...
    /// Записать батч f32-сэмплов (конвертируя в s16). Ошибки записи глотаем (не роняем звук).
    ...
    /// Досоставить размеры в заголовке (RIFF-size и data-size).
...
mod tests {
    ...
    fn writes_valid_wav_header_and_sizes() {
        let path = std::env::temp_dir().join("hw-rec-test.wav");
        {
            ...
            w.write(&[0.0, 0.5, -0.5, 1.0, -1.0]); // 5 сэмплов → 10 байт данных
        } // Drop финализирует заголовок
        ...
        // data size = 10 байт, RIFF size = 36 + 10
